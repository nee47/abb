...
// ESTA IMPLEMENTACINO NO FUNCIONA :C
// ENCONTRE ERRORES EN EL CASO2
...
// Funcion del 3er caso de borrado (el arbol tiene hijo derecho o hijo izquierdo).
...
void* caso_2(abb_nodo_t** arbol, abb_t* abb){
  ...
  if((*arbol)->derecha && !(*arbol)->izquierda) { /* 3er caso   */
    dato_salida = (*arbol)->dato;
    //abb_nodo_t* de = (*arbol)->derecha;
    //(*arbol)->izquierda = de->izquierda; //distinto
  ...
  if (!(*arbol)->derecha && (*arbol)->izquierda){
    dato_salida = (*arbol)->dato ;
    //abb_nodo_t* iz = (*arbol)->izquierda;
    //(*arbol)->derecha = iz->derecha; // distinto
...
void *abb_borrar(abb_t *abb, const char *clave){
  ...
  if((*arbol)->derecha && (*arbol)->izquierda){
    ...
    void* datex =  (*arbol)->dato;
    //void* aux_dato = datex;
    char* aux_clave = strdup((*arbol)->clave);
    free((*arbol)->clave);
    (*arbol)->dato = (*padre_minimo)->dato;
    //free((*arbol)->clave);
    abb_nodo_t* iz = (*padre_minimo)->izquierda; 
    (*arbol)->clave = strdup(iz->clave);
    (*padre_minimo)->izquierda->dato = datex;
    free((*padre_minimo)->izquierda->clave);
    (*padre_minimo)->izquierda->clave = strdup(aux_clave);
    free(aux_clave);
    //   abb_nodo_t** nodex = buscar_arbol(clave, &(padre_minimo)->derecha, abb->cmp);
    caso_2(&(*padre_minimo)->izquierda, abb);
    return dato_salida;
    /*
